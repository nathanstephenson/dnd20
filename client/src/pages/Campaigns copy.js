...
export default class Campaigns extends React.Component {
    ...
    constructor(props) {
        super(props)
        this.state = {selected:null, chosen:false, wantsNew:false, newName:"", newSubmitted:false, needsRefresh:false, wantsJoin:false, joinName:""}
        ...
        return(
            <>
                {!this.state.chosen && <><h1 className="title"> Campaigns </h1>
                    {!this.state.wantsJoin && <button onClick={()=>{this.setState({wantsJoin:true})}}>Join Campaign</button>}
                    {this.state.wantsJoin && <><input type="campaign"></input><button onClick={()=>{this.setState({wantsJoin:false})}}>x</button></>}
